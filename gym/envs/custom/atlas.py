...
class AtlasEnv(gym.Env):
  ...
  def __init__(self, render=False):
    ...
    dof_max_force = []
    active_joint = 0
    for j in range (p.getNumJoints(self.atlas)):
        info = p.getJointInfo(self.atlas, j)
        joint_name = info[1]
        joint_type = info[2]
        link_name = info[12]
  ...
  def step(self, action):
    ...
    p.setJointMotorControlArray(self.atlas, self.dof_idx, p.TORQUE_CONTROL,
                                forces=force)
    p.stepSimulation()
  ...
  def get_contact_forces(self):
    ...
    for foot_idx, foot_name in enumerate(self.feet):
        contact_result = p.getContactPoints(bodyA=self.atlas, linkIndexA=self.link_list[foot_name])
  ...
  def reset(self):
    ...
    p.changeDynamics(self.atlas, -1, linearDamping=0, angularDamping=0)
    # self.timeStep = 0.01
    self.timeStep = 1./240.
    p.setJointMotorControlArray(self.atlas, self.dof_idx, p.TORQUE_CONTROL,
    ...
    for i, dof_idx in enumerate(self.dof_idx):
        p.resetJointState(self.atlas, dof_idx, self.ini_pos[i] + randpos[i],
                          randvel[i])
    base_pos, base_quat, q, base_vel, base_so3, qdot = self.get_state()
  ...
  # ============================================================================
  def get_state(self):
    joint_states = p.getJointStates(self.atlas, self.dof_idx)
    q = [state[0] for state in joint_states]
    qdot = [state[1] for state in joint_states]
    base_pos, base_quat, base_vel, base_so3 = (), (), (), ()
    base_pos, base_quat= p.getBasePositionAndOrientation(self.atlas)
    base_vel, base_so3 = p.getBaseVelocity(self.atlas)
  ...
  def get_link_com_pos(self, link_idx):
    return p.getLinkState(self.atlas, link_idx,
